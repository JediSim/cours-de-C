...
% faire une action
faire(petit_dejeuner) :-
    position_courante(P),
    position(petit_dejeuner, P),
    action(petit_dejeuner,non),
...
faire(douche) :-
    ...
    position(douche, P),
    action(douche,non),
    retract(action(douche, non)),
    assert(action(douche, oui)),
    retract(score(Scr)),
    sous(Scr,Nscr,1000),
    assert(score(Nscr)),
    write("action faite."), nl,
...
decrire(salon) :-
        ...
        write("derrière vous se trouve la porte de votre chambre, en face la porte d'entrée pour aller à la gare. Vous hésitez a prendre un petit déjeuner, vos risquez d'être en retard..."),nl.
...
decrire(eve) :-
        write("[[ EVE ]]"),nl,
        write("Il n'y as personne... la sortie est derrière vous"),nl.
